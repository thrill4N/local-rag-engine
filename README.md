...
data/*.pdf, *.txt              (user-supplied documents)
  ...
  build_prompt()                 stuff top-k chunks into a grounding prompt
      │
      ▼
  ollama.chat (llama3.2)         streamed, cited generation
      │
      ▼
...
**Next step:** a faithfulness/groundedness check that verifies the model's
cited source actually matches a retrieved chunk, not just that citation
brackets are present syntactically (the current `grounded` flag in
`logger.py` is a cheap heuristic with a known blind spot: it can't catch
a hallucinated citation to a file that was never retrieved).
...
**Next step:** rotation/retention policy, and promoting the `grounded`
heuristic to the faithfulness check above once logs are being used to
make real quality-monitoring decisions.
...
1. **Reranking** — wider initial retrieval (`top_k=20`) + cross-encoder
   rerank before the final top-k reaches the LLM. Usually the single
   highest-ROI retrieval quality improvement available.
2. **Faithfulness scoring** — replace the bracket-presence `grounded`
   heuristic with a check that the cited source actually matches a
   retrieved chunk.
3. **Hybrid search (BM25 + vector)** — catches exact terms (case numbers,
   drug names, statute citations) that embeddings alone can miss.
4. **Structure-aware chunking** — respect document layout instead of
   character windows.
5. **Web UI** — local, `localhost`-bound, no data leaves the machine;
